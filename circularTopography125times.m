function circularTopography125times(MatrixData, lengthTime, timestep, legends)
...
%%  2. Plotting direct (positive) genaralization connectivity
    ...
    if sum(sum(MatrixData > 0))
        ...
        % Find non-zero values of s and their indices
          ...
          row = row(indsV);
          col = col(indsV);
          %%% Activate if you want white to color lines %colormapwhite2black = [linspace(1,0.35, length(v))' linspace(1,0.35, length(v))' linspace(1,0.35, length(v))'];
...
%%  3. Plotting inverse (negative) genaralization connectivity
    ...
    if sum(sum(MatrixData < 0))
        ...
        row = row(indsV);
        col = col(indsV);
        %%% Activate if you want white to color lines %colormapwhite2red = [linspace(1,0.7, length(v))' linspace(1,0.35, length(v))' linspace(1,0.35, length(v))'];
