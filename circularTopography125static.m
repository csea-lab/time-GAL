function circularTopography125static(MatrixData, mask, legends)
...
%%  2. Plotting direct (positive) genaralization connectivity
    ...
    if sum(sum(mask > 0))
        ...
        % Find non-zero values of s and their indices
          ...
          row = row(indsV);
          col = col(indsV);
          %% Activate this one if you want white to color %colormapwhite2black = [linspace(1,0.35, length(v))' linspace(1,0.35, length(v))' linspace(1,0.35, length(v))'];
...
%%  3. Plotting inverse (negative) genaralization connectivity
    ...
    if sum(sum(mask < 0)) % check if exist negative links
        ...
        % Find non-zero values of s and their indices
          ...
          row = row(indsV);
          col = col(indsV);
          %% Activate this one if you want white to color %colormapwhite2red = [linspace(1,0.7, length(v))' linspace(1,0.35, length(v))' linspace(1,0.35, length(v))'];
